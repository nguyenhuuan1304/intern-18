...
const AddProduct: React.FC<AddProductProps> = ({
  ...
  const stockData = product.inventory.reduce<Record<string, number>>(
    (acc, item) => {
  ...
  return (
    <>
      <div className="p-6 sm:p-2 w-full sm:w-[90%] md:w-[500px] mx-auto flex flex-col">
        <p className="text-xl font-bold mb-4 text-cyan-800 text-center">
          {product.name}
        </p>
          ...
          <table className="w-full border-collapse border border-gray-300 mb-4">
            <thead>
              <tr className="bg-gray-100">
                <th className="border border-gray-300 px-2 py-1 sm:px-4 sm:py-2 text-sm sm:text-base">
                  Sản phẩm
                </th>
                <th className="border border-gray-300 px-2 py-1 sm:px-4 sm:py-2 text-sm sm:text-base">
                  Tồn kho
              ...
              {Object.entries(stockData).map(([size, stock]) => (
                <tr key={size} className="text-sm sm:text-base">
                  ...
                  <td className="border border-gray-300 px-2 py-1 sm:px-4 sm:py-2 flex items-center justify-center">
                    <button
                      className="px-2 py-1 border border-gray-400 rounded text-xs sm:text-sm"
