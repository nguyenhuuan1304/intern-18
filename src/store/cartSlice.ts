...
export const addToCartApi = createAsyncThunk(
  "cart/addToCart",
  async (cartItem: CartItem, { getState, dispatch, rejectWithValue }) => {
    try {
      const state = getState() as { cart: CartState };

      // Kiểm tra sản phẩm có cùng tên & size trong giỏ hàng không
      const existingItem = state.cart.items.find(
        (item) => item.name === cartItem.name && item.size === cartItem.size
      );

      if (existingItem) {
        // nnếu đã có sản phẩm thì sẽ cập nhật số lượng
